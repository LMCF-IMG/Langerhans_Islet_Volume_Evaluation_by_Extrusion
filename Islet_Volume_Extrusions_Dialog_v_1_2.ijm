...
print("\\Clear");
if (modelName == "Vmod")
	print("Image,Pixel Size [µm/px],Minimum Islet Size [µm],Islet ID,Islet Size [µm],Area [µm2],Mean,Vmod [µm3],Vmod [nl],Vmod [IEQ],Vsphe [nl],Vsphe [IEQ],Vrico [nL],Vrico [IEQ]");
else
	print("Image,Pixel Size [µm/px],Minimum Islet Size [µm],Islet ID,Islet Size [µm],Area [µm2],Mean,Vext [µm3],Vext [nl],Vext [IEQ],Vsphe [nl],Vsphe [IEQ],Vrico [nL],Vrico [IEQ]");

//print("areaIslet = " + areaIslet);
...
function doProcessingMorphoLibJ(filename) { 
	...
	selectImage("Labels");
	// First, filling all empty objects in firstly found Labels
	...
	if (roiCount == 0) {
	    exit("ROI Manager je prázdný.");
	}
	// Smyčka přes všechny ROI
	for (i = 0; i < roiCount; i++) {
	    // Duplikace obrazu pro práci
	    selectWindow("Masks_from_Labels");
	   	run("Select None");
	    run("Duplicate...", "title=temp");
	    selectWindow("temp");
	    // Vymazání všeho mimo aktuální ROI
	    roiManager("Select", i);    
	    run("Clear Outside");	    
	    // Vyplnění děr
	    run("Fill Holes");
		...
		roiManager("Select", i);
	    run("Copy");
		// Vložení zpět do původního obrazu
	    selectWindow("Masks_from_Labels");
	    roiManager("Select", i);
	    run("Paste");
	    // Zavření dočasného okna
	    close("temp");
	}
	...
	success = File.delete(path);				// smazani pomocneho souboru
	
	lines = split(content, "\n");				// Rozděl string podle řádků
    indices = newArray(roiManager("count"));
    for (i = 0; i < indices.length; i++)
    	indices[i] = 0;

	for (i = 1; i < lines.length; i++) {		// přeskoč záhlaví, tj. od 1
    	line = trim(lines[i]);
    	if (line == "") continue; 				// přeskoč prázdné řádky
    		values = split(line, "\t");			// Rozděl řádek podle tabulátoru
    	if (values.length >= 3) {				// Získej druhou a třetí hodnotu (indexy 1 a 2)
        	area = parseFloat(values[1]);
        	circularity = parseFloat(values[2]);        		        
	        // hodnoceni area a cirkularity; v ROI zustane jen to, co splnuje obe podminky	        
			if (area < areaIslet || circularity < circularityMin)
    			indices[i-1] = 1; 				// označí prvek k !odstranění z ROI
    	}
	}
	
	// Iterujeme pozpátku, abychom zachovali správné indexy při mazání
	...
	for (i = 0; i < roiManager("count"); i++) {
		...
		print(title + "," + pixelSizeStr + "," + isletDiamStr + "," + (i+1) + "," + d2s(2*circleRadius, 3) + 
				...
				"," + d2s(ricordi_nL, 3) + "," + d2s(ricordi_IEQ, 3));
	}
